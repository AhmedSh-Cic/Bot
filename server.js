...
setInterval(() => {
  http.get(`http://impactsora.glitch.me/`);
}, 280000);
...
const youtube = new YouTube("AIzaSyABVxsbCsPVgozZziDz4oFaCMRKVvKLpW4"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyABVxsbCsPVgozZziDz4oFaCMRKVvKLpW4"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@448191712846675978>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content === "Hi") {
    message.channel.send("**:heart:Hi:heart:**");
    message.channel.sendFile("");
  }
});
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setby")) {
    ...
    client.on("guildMemberAdd", member => {
      ...
      var channel = member.guild.channels.find(gg => gg.name === "┊꒰🔐꒱┊warning-and-mute"); //// تعديل اساسي
      if (!channel) return;
...
client.on("message", message => {
  if (message.content.split(" ")[0] === prefix + "invite") {
    ...
    const Embed11 = new Discord.RichEmbed().setColor("RANDOM")
      .setDescription(`** Link valid for:1 day,With 100 max number of uses **`);
